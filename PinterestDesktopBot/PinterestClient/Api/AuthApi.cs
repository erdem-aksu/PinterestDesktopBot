...
namespace PinterestDesktopBot.PinterestClient.Api
{
    internal class AuthApi : IAuthApi
    {
        ...
        {
            new {name = "simplified_homepage_modal.signup.click"},
        ...
        private static readonly List<object> ThirdStepActions = new List<object>()
        {
            ...
            new {name = "web_funnel.view_homepage-press_signup-focus_password-press_signup_continue"},
        };
        ...
        public async Task RegisterAsync(RegisterInput input)
        {
            ...
            await Api.PostAsync(PinterestApiConstants.ResourceRegistrationComplete,
                new {placed_experience_id = RegistrationCompleteExperienceId, extra_context = new object()});
        ...
        public async Task<AutoRegisteredAccount> AutoRegisterAsync(string country = "US", string locale = "en-US")
        {
            ...
            var imageUrl = await Api.UploadAsync(avatar, Guid.NewGuid() + Path.GetExtension(avatarUrl), "img", false);
        ...
        public async Task<AutoRegisteredAccount> AutoRegisterPersonalAsync(string mailProvider = null,
            string country = "US", string locale = "en-US")
        {
            ...
            {
                var vestaClient =
                    new global::VestaClient.VestaClient("https://116.203.221.31:8083/", "admin", "Ea1021+2015");

                await vestaClient.Mail.CreateMailDomain(mailProvider);
                await vestaClient.Mail.CreateMailAddress(mailProvider, email.Split('@')[0], password);
            }
        ...
        public async Task<AutoRegisteredAccount> AutoRegisterPersonalToBusinessAsync(string mailProvider = null,
            string country = "US", string locale = "en-US")
        {
            var faker = new Faker();
            ...
            await ConvertToBusinessAsync(businessName, string.Empty, false);
        ...
        public async Task ResendValidationEmailAsync(bool login = true)
        {
            await Api.PostAsync(PinterestApiConstants.ResourceEmailResend, new {}, login);
        }
