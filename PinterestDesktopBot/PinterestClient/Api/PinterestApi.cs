...
namespace PinterestDesktopBot.PinterestClient.Api
{
    internal class PinterestApi
    {
        ...
        public async Task<string> UploadAsync(byte[] bytes, string filename, string name = "img",
            bool shouldLogin = true)
        {
            using (var response = await Client
                .UploadAsync(PinterestApiConstants.ImageUpload, bytes, filename, name, shouldLogin)
                .ConfigureAwait(false))
            {
                if (!response.IsSuccessStatusCode)
                {
                ...
                return res.GetValue("image_url")?.ToString();
            }
        }

        public async Task<T> PostAppAsync<T>(string path, object value, bool shouldLogin = false,
            object options = null, JsonSerializerSettings serializerSettings = null)
        {
            using (var response = await Client.PostAppAsync(path, value, options, shouldLogin, serializerSettings)
                .ConfigureAwait(false))
            {
                if (!response.IsSuccessStatusCode)
                {
        ...
        public async Task<string> PatchAppAsync(string path, object value, bool shouldLogin = false,
            object options = null, JsonSerializerSettings serializerSettings = null)
        {
            using (var response = await Client.PatchAppAsync(path, value, options, shouldLogin, serializerSettings)
                .ConfigureAwait(false))
            {
                if (!response.IsSuccessStatusCode)
                {
        ...
        {
            Client.SetUser(user, isLoggedIn);
        }
        ...
        {
            Client.SetStateData(stateData);
        }
        
        public ProxyData GetProxyData()
        {
            return Client.GetProxyData();
        }

        public void SetProxyData(ProxyData proxyData)
        {
            Client.SetProxyData(proxyData);
        }
        ...
        {
            return await Client.GetAppAccessToken(appId, secret);
        }

        public async Task<string> GetAccessTokenNew(List<Cookie> cookies = null, bool auth = true)
        {
            if (cookies == null)
            {
                return await Client.GetAccessTokenNew(auth);
            }
            
            return await Client.GetAccessTokenNew(cookies);
        }

        public async Task<string> GetAccessTokenNewV2(bool auth = true)
        {
            return await Client.GetAccessTokenNewV2(auth);
        }
